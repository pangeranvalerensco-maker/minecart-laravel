...
## [2026-06-29] Sprint 7: Fitur Live Chat, Lacak Resi, & Pembayaran Manual

### Pekerjaan yang Dilakukan:
1. **Perbaikan UX Global**:
   - Menambahkan aturan CSS global untuk `cursor: pointer` pada semua tombol, tautan, dan ikon keranjang agar interaktif saat di-*hover*.
2. **Perubahan Sistem Pembayaran (Sementara)**:
   - Menghapus konfigurasi Snap Midtrans dari `CheckoutController` dan mengubah metode ke `manual_transfer`.
   - Membuat `UploadProofController` agar pembeli bisa mengunggah bukti transfer.
   - Menambahkan status `pending_verification` pada pesanan sehingga Penjual dapat melihat bukti transfer tersebut dan menekan tombol **Terima Pembayaran** pada halaman dasbor penjual.
3. **Penyelesaian Fitur Lacak Resi**:
   - Menerapkan fitur bagi Penjual untuk meng-*input* nomor resi ke dalam `OrderItem`.
   - Mengubah status pesanan menjadi `shipped` dan menampilkan informasi kurir serta resi di halaman Riwayat Pesanan Pembeli.
4. **Penyelesaian Fitur Live Chat**:
   - Menyesuaikan `products/show.blade.php` agar tombol chat mengarah ke `user_id` pemilik produk (Seller).
   - Menyempurnakan antarmuka chat (`chat.index`) menggunakan teknik *AJAX polling* dasar setiap 3 detik.
   - Menjalankan migrasi `conversations` dan `messages`, serta membuat relasinya untuk memungkinkan penjual dan pembeli berinteraksi secara *real-time*.

### Catatan untuk Sesi Selanjutnya:
- **Phase 2 (Selesai)**: Sistem Ulasan, Lacak Resi, Live Chat, dan Pembayaran Manual.
- **Midtrans**: Pengguna meminta integrasi Midtrans dilewati sementara untuk perapian tampilan, namun **tetap ingin menggunakannya di masa depan**. Pastikan untuk melihat kembali `MASTER_PLAN.md` Fase 3 ketika ingin menghidupkannya lagi.
- **Fase 3**: Fokus berikutnya adalah sistem diskon, *Flash Sale*, dan *Wishlist*.
